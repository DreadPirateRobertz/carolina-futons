# CF Project Progress Report
**Auto-refreshed every 10 min | Last updated: 2026-05-03 19:20 MT**

---

## Vercel ↔ Wix Integration — How It Works

### Product Updates (Live — No Manual Import Needed)
- **cfW queries Wix Stores live** via `@wix/sdk` — new products in Wix Dashboard appear in cfW **immediately**
- PDP: two-step fetch (slug→ID→full product with variants). PLP: paginated by collection (≤48). Search: full-catalog scan ≤500.
- **Wix Dashboard → Wix Stores** is the single source of truth for inventory, pricing, images, variants

### Velo Backend → cfW (Vercel)
```
cfW Server Action → POST https://www.carolinafutons.com/_functions/<endpoint>
                          ↓ Wix Velo HTTP function (http-functions.js) ↓ wixData/webMethod
```
Active: `trackCustomEvent` ✅ `sampleRequests` ✅ `notifyMe` ✅ `deliveryZone` ✅ `contactSubmissions` ⚠️ (prod sync pending) `crossRigEventReceiver` ✅

### Cross-Rig (Mobile ↔ cfW)
Channel A dual-write active: mobile fires both cfW `/api/cross-rig` + Wix concurrently. CROSS_RIG_SECRET: **Vercel Production ✅ + Wix Staging ✅ + Mobile ✅** (dallas confirmed). DNS cutover pending cf-cb9s — §5 order-lookup 501 still open.

---

## Session Merges — SHIPPED TO VERCEL ✅

| PR | What | Merged |
|----|------|--------|
| #289 | fix(cf-ml6n): doubled footer removed | earlier session |
| #315 | feat(cm-002): AR model-viewer on PDP mobile | earlier session |
| #327 | fix(header): LivingSky backdrop removed | earlier session |
| #334 | fix(cf-o8s9): restore LivingSky backdrop ← Stilgar directive | this session |
| #328 | feat(cf-shop-mascot): animated bear/deer/fox/owl category cards | earlier session |
| #321 | fix(cf-ac1y): replace stale ShopTheRoom product slugs | 18:57 MT |

**Velo also merged:** #1132 (blog post count fix) ✅ · #1131 (one-click unsubscribe) ✅

**Main:** E2E Option A (30s timeout, 881708c) + Option B (721ca82, E2E skipped on PRs) both live.

**Vercel deploys from main — changes live on Vercel preview URL.**

---

## CF Open PRs (carolina-futons / Velo)

| PR | Title | CI |
|----|-------|----|
| #1133 | feat(cf-y2l3): trade-in / trade-up program | ❌ fail |
| #1130 | chore(deps): dev-deps bump | **HOLD** |
| #1128 | chore(deps): postcss bump | ✅ pass — merge eligible |
| #1125 | feat(cf-9t70): sampleRequests endpoint | ❌ fail (codecov only) |
| #1120 | feat(cf-3qt.4.4): delivery zone distance calc | ❌ fail |

---

## CFW Open PRs (carolina-futons-web / Next.js)

| PR | Title | CI |
|----|-------|----|
| #337 | fix(cf-p7la): prevent hydrateCartAction racing addItemAction | ✅ pass |
| #336 | feat(cf-9cgu): LivingSkyErrorBoundary | ✅ pass — merge eligible |
| #335 | fix(cf-cfol): guard checkout against cart-write race | ✅ pass |
| #332 | feat(cf-xmpb): commerce dark mode — PDP + account | ✅ pass |
| #331 | feat(cf-4bhw): Gift Registry — /registry + /registry/[slug] | ❌ fail |
| #330 | feat(cf-wzl3): LivingSky forced night state in dark mode | ✅ pass |
| #329 | feat(cf-8w86): Gift Cards page — /gift-cards route | ✅ pass |
| #326 | fix(cf-kj8n): VERCEL_PROJECT_PRODUCTION_URL sitemap | ✅ pass |
| #323 | feat(cf-7dfv): wire PdpFinancing into PDP | ✅ pass |
| #322 | fix(cf-j6ub): useTimeOfDay RAF + SSR flash guard | ✅ pass |
| #320 | feat(cf-kjpy): Local SEO city pages | ✅ pass — merge eligible |
| #319 | feat(cf-3i8j): 2D drag-drop room planner | ❌ fail |
| #318 | feat(cf-footer): consolidate footer living illustration | ✅ pass |
| #317 | fix(cf-m80l): Canby collection slug | ✅ pass |
| #303 | feat(cf-u7yk): /gift-cards page (older, dupes #329) | ❌ fail |
| #299 | fix(cf-urbq): dark mode font contrast | ❌ fail |
| #296 | feat(cf-e4vd/cf-ph80): HomeQuizCta + Swatch | ✅ pass |
| #293 | feat(cf-urfn): HomeSaleStrip | ✅ pass |
| #291 | feat(cf-ww8u): PdpSizeGuide | ❌ fail |
| #290 | feat(cf-lqnd): PDP back-in-stock notify me | ✅ pass |
| #282 | feat(cf-0y1e): PDP Size Guide v2 | ✅ pass |
| #281 | feat(cf-7axq): Add to Compare | ❌ fail |
| #278 | feat(cf-c7re): HTTP security headers | ✅ pass |
| #276 | test(cf-o3bv): CartDrawer+CartPage sentinel | ✅ pass |
| #136 | docs(cf-93rb-B): design-tokens delta [DRAFT] | ✅ pass |

**❌ fail = lint/typecheck failure (E2E skipped on PR builds since 721ca82).**

**Merge queue (passing CI, no conflicts):** #336, #320, #332, #330, #329, #326, #323, #322, #318, #317, #296, #293, #290, #282, #278, #276

---

## Crew Assignments

| Crew | Current Task | Bead |
|------|-------------|------|
| blaidd | Cart QA seeded fixtures — dispatched this turn | cf-9izd |
| godfrey | PDP Financing BNPL — PR #323 passing, check + merge | cf-10fx |
| radahn | Local SEO — PR #320 passing, final review + signal | cf-kjpy |
| rennala | Sitemap 404 investigation — /sitemap.xml on prod | cf-rb07 |
| millicent | Illustration wiring — wire v1/v2/v3 into sub-pages | cf-tyuk |
| morgott | Dark mode coordination — PRs #330/#332 ready | cf-rymw |
| miquella | Review PRs #335/#337 (cart convoy), merge #336 | cf-9cgu |

---

## Open Blockers

| Issue | Status |
|-------|--------|
| **P0 cart regression** (cf-cfol) | PR #335 passing CI — awaiting miquella review |
| **P1 cart race** (cf-p7la) | PR #337 passing CI — awaiting miquella review |
| **LivingSky ErrorBoundary** (cf-9cgu) | PR #336 passing — miquella to merge |
| **Illustration wiring** (cf-tyuk) | millicent dispatched — v1/v2/v3 into sub-pages |
| **contactSubmissions 404** | Velo endpoint exists; awaiting Stilgar §1-§3 DNS clearance |
| **DNS flip** (cf-cb9s) | Stilgar manual §1-§3 pending; §5 order-lookup 501. Dallas holding. |
| **CROSS_RIG_SECRET** | ✅ COMPLETE: Vercel ✅ Wix ✅ Mobile ✅ |
| **LivingSky header** | ✅ PR #334 merged — Stilgar directive complete |

---

## In-Progress Beads (14 total)

| Bead | Pri | Title |
|------|-----|-------|
| cf-cfol | P1 | [P0] Add-to-cart race condition — PR #335 pending review |
| cf-p7la | P1 | [P1] Sticky add-to-cart race — PR #337 pending review |
| cf-10fx | P1 | PDP Financing / BNPL section — PR #323 passing |
| cf-d3hc | P1 | PDP Financing / BNPL section (Afterpay, Affirm) |
| cf-9t70 | P1 | /swatch-request page — fabric sample order form |
| cf-c77s | P1 | E2E auto emails + challenges + reward system |
| cf-kjpy | P1 | cfW Local SEO city pages — PR #320 passing |
| cf-rtd7 | P1 | cf-3qt full prod parity audit |
| cf-yfvl | P1 | cf-theme-experiments — A/B/C/D home variants |
| cf-tyuk | P1 | Wire v1/v2/v3 illustrations into sub-pages — millicent |
| cf-9fd8 | P2 | PDP back-in-stock notify me |
| cf-9izd | P2 | cf-3qt cart QA: seeded-fixture preview deployment |
| cf-rymw | P2 | cf-dark-mode — site-wide dark mode option |
| cf-0s4l | P3 | /sustainability — wire Wix CMS |
| cf-rb07 | P3 | SEO: /sitemap.xml 404 on prod |

---

## Nightly CI
| Repo | Schedule | State |
|------|---------|-------|
| carolina-futons | `0 6 * * *` | ✅ Running |
| carolina-futons-web | `0 6 * * *` | ✅ Fixed — E2E 30s timeout + skip on PRs both live |
| carolina-futons-stage3-velo | `0 6 * * *` | ✅ Running |

---
*Cron: 1709bbed · Next refresh ~10 min*
