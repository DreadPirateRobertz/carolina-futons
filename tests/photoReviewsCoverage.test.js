...
// ── getPhotoReviewStats error catch block (lines 298-299) ─────────────────────

describe('getPhotoReviewStats — error catch block', () => {
  it('returns failure response when find() throws (covers lines 298-299)', async () => {
    vi.spyOn(wixData, 'query').mockImplementation(() => ({
      eq() { return this; },
      hasSome() { return this; },
      limit() { return this; },
      async find() { throw new Error('Database connection lost'); },
    }));

    const result = await getPhotoReviewStats(VALID_PRODUCT_ID);

    expect(result.success).toBe(false);
    expect(result.error).toBe('Failed to load review stats.');
...
describe('getPhotoReviewStats — rating distribution accuracy', () => {
  it('correctly counts each star bucket', async () => {
    __seed('PhotoReviews', [
      { _id: 'r1', productId: 'a0b1c2d3', rating: 1, photoUrl: 'img.jpg', status: 'approved' },
      { _id: 'r2', productId: 'a0b1c2d3', rating: 2, photoUrl: 'img.jpg', status: 'approved' },
      { _id: 'r3', productId: 'a0b1c2d3', rating: 2, photoUrl: 'img.jpg', status: 'approved' },
      { _id: 'r4', productId: 'a0b1c2d3', rating: 3, photoUrl: 'img.jpg', status: 'approved' },
      { _id: 'r5', productId: 'a0b1c2d3', rating: 4, photoUrl: 'img.jpg', status: 'featured' },
      { _id: 'r6', productId: 'a0b1c2d3', rating: 5, photoUrl: 'img.jpg', status: 'featured' },
      { _id: 'r7', productId: 'a0b1c2d3', rating: 5, photoUrl: 'img.jpg', status: 'approved' },
    ]);

    const result = await getPhotoReviewStats('a0b1c2d3');

    expect(result.success).toBe(true);
    expect(result.stats.totalReviews).toBe(7);
    expect(result.stats.ratingDistribution[1]).toBe(1);
    expect(result.stats.ratingDistribution[2]).toBe(2);
    expect(result.stats.ratingDistribution[3]).toBe(1);
    expect(result.stats.ratingDistribution[4]).toBe(1);
    expect(result.stats.ratingDistribution[5]).toBe(2);
  });

  it('clamps out-of-range ratings into valid buckets', async () => {
    __seed('PhotoReviews', [
      // source clamps: Math.min(5, Math.max(1, r.rating || 5))
      { _id: 'r1', productId: 'a0b1c2d3', rating: 99, photoUrl: 'img.jpg', status: 'approved' },
      { _id: 'r2', productId: 'a0b1c2d3', rating: -1, photoUrl: 'img.jpg', status: 'approved' },
    ]);

    const result = await getPhotoReviewStats('a0b1c2d3');

    expect(result.success).toBe(true);
    // 99 clamps to 5, -1 clamps to 1
    expect(result.stats.ratingDistribution[5]).toBe(1);
    expect(result.stats.ratingDistribution[1]).toBe(1);
  });

  it('only counts reviews with non-empty photoUrl in photoCount', async () => {
    __seed('PhotoReviews', [
      { _id: 'r1', productId: 'a0b1c2d3', rating: 5, photoUrl: 'img.jpg', status: 'approved' },
      { _id: 'r2', productId: 'a0b1c2d3', rating: 4, photoUrl: '', status: 'approved' },
      { _id: 'r3', productId: 'a0b1c2d3', rating: 3, photoUrl: null, status: 'featured' },
    ]);
    ...
    expect(result.stats.photoCount).toBe(1); // only r1 has truthy photoUrl
  });

  it('counts featuredCount correctly across mixed statuses', async () => {
    __seed('PhotoReviews', [
      { _id: 'r1', productId: 'a0b1c2d3', rating: 5, photoUrl: 'img.jpg', status: 'featured' },
      { _id: 'r2', productId: 'a0b1c2d3', rating: 4, photoUrl: 'img.jpg', status: 'approved' },
      { _id: 'r3', productId: 'a0b1c2d3', rating: 3, photoUrl: 'img.jpg', status: 'featured' },
    ]);

    const result = await getPhotoReviewStats('a0b1c2d3');

    expect(result.success).toBe(true);
    expect(result.stats.featuredCount).toBe(2);
  });
});
