...
describe('COLLECTION_MANIFEST', () => {
  it('should contain exactly 21 collections', () => {
    expect(COLLECTION_MANIFEST).toHaveLength(21);
  });
...
describe('provisionCollections', () => {
  ...
  it('should create all 21 missing collections', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledTimes(22); // 1 list + 21 creates
  });
  ...
  it('should report errors for failed creates without stopping', async () => {
    ...
    expect(results.filter((r) => r.status === 'CREATED')).toHaveLength(20);
  });
  ...
  it('should handle mixed existing and missing collections', async () => {
    ...
    expect(results.filter((r) => r.status === 'CREATED')).toHaveLength(11);
