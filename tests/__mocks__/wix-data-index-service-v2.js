...
export function __setCreateError(error) { _createError = error; }
export function __setListError(error) { _listError = error; }
...
export const indexes = {
  async listIndexes(collectionId) {
    if (_listError) { const err = _listError; _listError = null; throw err; }
    return { indexes: _indexes[collectionId] || [] };
  },

  async createIndex(collectionId, index) {
    if (_createError) { const err = _createError; _createError = null; throw err; }
