...
describe('trackBundleImpression', () => {
  it('rejects invalid bundle ID', async () => {
    const r = await mod.trackBundleImpression({ bundleId: null, event: 'impression' });
    expect(r.success).toBe(false);
  });

  it('rejects invalid event type', async () => {
    const r = await mod.trackBundleImpression({ bundleId: 'b1', event: 'invalid' });
    expect(r.success).toBe(false);
    expect(r.error).toContain('Invalid event');
  });

  it('tracks impression event', async () => {
    const r = await mod.trackBundleImpression({ bundleId: 'b1', bundleName: 'Test Bundle', event: 'impression' });
    expect(r.success).toBe(true);
    expect(_collections['BundleAnalytics']).toHaveLength(1);
    expect(_collections['BundleAnalytics'][0].event).toBe('impression');
  });

  it('tracks click event', async () => {
    const r = await mod.trackBundleImpression({ bundleId: 'b1', event: 'click' });
    expect(r.success).toBe(true);
  });

  it('tracks add_to_cart event', async () => {
    const r = await mod.trackBundleImpression({ bundleId: 'b1', event: 'add_to_cart' });
    expect(r.success).toBe(true);
  });

  it('tracks purchase event with revenue', async () => {
    const r = await mod.trackBundleImpression({ bundleId: 'b1', event: 'purchase', revenue: 599.99 });
    expect(r.success).toBe(true);
    expect(_collections['BundleAnalytics'][0].revenue).toBe(599.99);
  });

  it('sets revenue to 0 for non-purchase events', async () => {
    await mod.trackBundleImpression({ bundleId: 'b1', event: 'impression', revenue: 100 });
    expect(_collections['BundleAnalytics'][0].revenue).toBe(0);
  });

  it('defaults source to product_page for invalid source', async () => {
    await mod.trackBundleImpression({ bundleId: 'b1', event: 'impression', source: 'invalid' });
    expect(_collections['BundleAnalytics'][0].source).toBe('product_page');
  });

  it('accepts valid sources', async () => {
    for (const source of ['product_page', 'cart', 'homepage', 'category']) {
      _collections = {};
      await mod.trackBundleImpression({ bundleId: 'b1', event: 'impression', source });
      expect(_collections['BundleAnalytics'][0].source).toBe(source);
    }
  });

  it('includes memberId when authenticated', async () => {
    await mod.trackBundleImpression({ bundleId: 'b1', event: 'impression' });
    expect(_collections['BundleAnalytics'][0].memberId).toBe('member1');
  });

  it('handles anonymous user', async () => {
    _mockMember = null;
    await mod.trackBundleImpression({ bundleId: 'b1', event: 'impression', sessionId: 'sess123' });
    expect(_collections['BundleAnalytics'][0].memberId).toBe('');
    expect(_collections['BundleAnalytics'][0].sessionId).toBe('sess123');
  });
});
