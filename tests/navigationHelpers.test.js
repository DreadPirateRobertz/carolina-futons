...
describe('NAV_LINKS — Gift Cards entry', () => {
  ...
  it('#navGiftCards path is /gift-cards', () => {
    expect(NAV_LINKS['#navGiftCards'].path).toBe('/gift-cards');
  });

  it('#navGiftCards label is "Gift Cards"', () => {
    expect(NAV_LINKS['#navGiftCards'].label).toBe('Gift Cards');
  });

  it('getActiveNavId returns #navGiftCards for /gift-cards', () => {
...
describe('MEGA_MENU_CATEGORIES — Gift Cards in More group', () => {
  it('Gift Cards appears in MEGA_MENU_CATEGORIES', () => {
    const allItems = MEGA_MENU_CATEGORIES.flatMap(g => g.items);
    const item = allItems.find(i => i.id === '#navGiftCards');
    expect(item).toBeDefined();
  });

  it('Gift Cards item has correct path', () => {
    const allItems = MEGA_MENU_CATEGORIES.flatMap(g => g.items);
    const item = allItems.find(i => i.id === '#navGiftCards');
    expect(item.path).toBe('/gift-cards');
  });

  it('Gift Cards item has correct label', () => {
    const allItems = MEGA_MENU_CATEGORIES.flatMap(g => g.items);
    const item = allItems.find(i => i.id === '#navGiftCards');
    expect(item.label).toBe('Gift Cards');
  });

  it('Gift Cards is in the More category group', () => {
    const moreGroup = MEGA_MENU_CATEGORIES.find(g => g.title === 'More');
    expect(moreGroup).toBeDefined();
    const ids = moreGroup.items.map(i => i.id);
    expect(ids).toContain('#navGiftCards');
  });
});
