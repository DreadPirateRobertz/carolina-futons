  ...
  try {
    ...
    try {
      ...
      if (member) {
        ...
        const wishlistCheck = await wixDataMod.query('Wishlist')
          .eq('memberId', member._id)
          .eq('productId', currentProduct?._id)
          .find();

        if (wishlistCheck.items.length > 0 && wishlistCheck.items[0].muteAlerts !== true) {
          // Member has this in wishlist with alerts on — show enrolled state
          submitBtn.hide();
          emailInput.hide();
          if (successMsg) {
            successMsg.text = "You'll be notified when this item is back — it's in your wishlist.";
            successMsg.show();
          }
          return; // Skip email form setup
        }
      }
    } catch (e) {
      // Non-critical — fall through to email form
    }

    // Handle email submission (for guests and non-wishlisted members)
