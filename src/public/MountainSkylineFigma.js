 ...
 * Sky gradient and ridge colors update with time-of-day; no blur overlays.
 *
 * @param {Object} [ridgeColors] - LivingSkyState.ridgeColors: { r1, r2, r3, r4, tree }
 * @param {string[]} [skyColors]  - LivingSkyState.skyColors: [sky0, sky1, sky2, sky3]
 * @param {number} [height=120]
 * @returns {string} Complete SVG markup string.
 */
export function buildHeaderMountainSVG(ridgeColors, skyColors, height) {
  const rc = ridgeColors || {};
  const sc = Array.isArray(skyColors) && skyColors.length >= 4 ? skyColors : null;
  const h = (typeof height === 'number' && height > 0) ? height : DEFAULT_HEIGHT;
  const D = HEADER_DEFAULTS;

  const sky0 = safeHeaderColor(sc && sc[0], D.sky[1]);
  const sky1 = safeHeaderColor(sc && sc[1], D.sky[2]);
  const sky2 = safeHeaderColor(sc && sc[2], D.sky[3]);
  const sky3 = safeHeaderColor(sc && sc[3], D.sky[4]);

  // Map LivingSky ridgeColors to 7 ridge layers (far → near).
  const fills = [
    safeHeaderColor(rc.r4,   D.r[0]),
    safeHeaderColor(rc.r4,   D.r[1]),
    safeHeaderColor(rc.r3,   D.r[2]),
    safeHeaderColor(rc.r2,   D.r[3]),
    safeHeaderColor(rc.r2,   D.r[4]),
    safeHeaderColor(rc.r1,   D.r[5]),
    safeHeaderColor(rc.tree, D.r[6]),
  ];

  const defs = '<defs>'
    + '<linearGradient id="sky-grad-dyn" x1="0" y1="0" x2="0" y2="1">'
    + `<stop offset="0%" stop-color="${D.sky[0]}" stop-opacity="1"/>`
    + `<stop offset="20%" stop-color="${sky0}" stop-opacity="1"/>`
    + `<stop offset="50%" stop-color="${sky1}" stop-opacity="1"/>`
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="${h}" viewBox="0 0 1440 200" preserveAspectRatio="none" aria-hidden="true" role="presentation">`
    + defs
    + `<rect width="1440" height="200" fill="url(#sky-grad-dyn)"/>`
    + ridgeSvg
    + DECOR_SVG
    + '</svg>';
}
...
/**
 * Update #headerSkyline with LivingSkyState colors.
 * Called on each livingSkyFrame message to keep header in sync with sky.
 *
 * @param {Function} $w - Wix selector
 * @param {Object|null} state - LivingSkyState: { ridgeColors, skyColors, ... }
 * @param {Object} [options]
 * @param {string} [options.containerId='#headerSkyline']
 * @param {number} [options.height]
 */
export function updateHeaderSkylineWithState($w, state, options) {
  try {
    const opts = options || {};
    const containerId = opts.containerId || '#headerSkyline';
    const container = $w(containerId);
    if (!container) return;
    const rc = (state && state.ridgeColors) || {};
    const sc = (state && state.skyColors) || null;
    container.html = buildHeaderMountainSVG(rc, sc, opts.height);
  } catch (_e) {}
}
