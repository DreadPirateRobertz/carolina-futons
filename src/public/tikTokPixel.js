...
// PageView is NOT fired here — callers must use pixelConsentService.fireTrackedTikTokEvent('PageView', {}).
// Additional events (ViewContent, AddToCart, Purchase) can be fired via fireTikTokEvent().
//
 ...
 * after consent is granted. Safe to call on every page — guards against re-initialization
 * and missing browser globals.
 */
...
const _PII_FIELDS = ['email', 'phone', 'phone_number', 'userId', 'user_id', 'external_id', 'sha256_email', 'sha256_phone'];

/**
 * Fire a TikTok Pixel event.
 * PII fields (email, phone, userId, etc.) are stripped before sending — defence-in-depth.
...
export function fireTikTokEvent(eventName, params = {}) {
  try {
    ...
    const safeParams = Object.fromEntries(
      Object.entries(params).filter(([k]) => !_PII_FIELDS.includes(k))
    );
