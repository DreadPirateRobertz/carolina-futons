...
/**
 * Generate a room-prep checklist based on product dimensions.
 *
 * @param {Array} items - [{name, widthInches, depthInches, heightInches, weightLbs}]
 * @returns {{success: boolean, checklist: Array}}
 * @permission Anyone
 */
export const generateRoomPrepChecklist = webMethod(
  Permissions.Anyone,
  (items) => {
    ...
    const checklist = [];
    const maxWidth = Math.max(...items.map(i => i.widthInches || 0));
    const maxDepth = Math.max(...items.map(i => i.depthInches || 0));
    const totalWeight = items.reduce((sum, i) => sum + (i.weightLbs || 0), 0);
    const largestDim = Math.max(maxWidth, maxDepth);

    if (largestDim > STANDARD_DIMENSIONS.frontDoor.width) {
      checklist.push({
        priority: 'high',
        category: 'access',
        task: 'Measure your front door',
        detail: `Your largest item is ${largestDim}" wide. Standard doors are ${STANDARD_DIMENSIONS.frontDoor.width}".`,
      });
    }

    if (largestDim > STANDARD_DIMENSIONS.interiorDoor.width) {
      checklist.push({
        priority: 'high',
        category: 'access',
        task: 'Check interior doorways on the delivery path',
        detail: `Interior doors are typically ${STANDARD_DIMENSIONS.interiorDoor.width}" wide.`,
      });
    }
    ...
    if (totalWeight > 150) {
      checklist.push({
        priority: 'medium',
        category: 'safety',
        task: 'Have a helper available',
        detail: `Total delivery weight is ${totalWeight} lbs.`,
      });
    }
    ...
    checklist.push({
      priority: 'medium',
      category: 'room',
      task: 'Clear the placement area',
      detail: `You need at least ${maxWidth}"×${maxDepth}" of clear floor space plus 12" on each side.`,
    });
    ...
    if (totalWeight > 100) {
      checklist.push({
        priority: 'low',
        category: 'room',
        task: 'Protect floors along the delivery path',
        detail: 'Place blankets or cardboard on hardwood/tile floors.',
      });
    }
    ...
    checklist.push({
      priority: 'low',
      category: 'setup',
      task: 'Have basic tools ready',
      detail: 'Phillips screwdriver and Allen key (usually included).',
    });
