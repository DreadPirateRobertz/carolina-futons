...
|---|---|---|---|
| `deprecations` | Uses deprecated APIs | **All 10 audits** | Some third-party script (likely Wix Headless SDK or analytics) still calls a deprecated browser API. Inspect the report HTML to identify the API. Drives the BP=81 ceiling everywhere. |
...
| `meta-description` | Document does not have a meta description | kingston PDP (D+M) only | PDP `<head>` is missing `<meta name="description">`. Drives PDP SEO=92. Add a templated meta description from the product description. |
